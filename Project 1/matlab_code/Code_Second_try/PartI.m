...
for i = 1:length(alt_range)
    ...
    for j = 1:length(mach_range)
        ...
        [Tex, Sep, alfa_val] = TexSep(alt_range(i), mach_range(j), mass, gamma_rad);
...
h1 = plot(mach_range, curves(1).thrusts, ...
    plot_config{1}, 'LineWidth', 2); % first and last bolted
...
%% alfa - qdyn limits actual plotting
figure(1);
hold on
alfa_lim1 = plot(alfa_mach, alfa_tex, 'rx', 'LineWidth', 2);
qdyn_lim1 = plot(qdyn_mach, qdyn_tex, 'bx', 'LineWidth', 2);
